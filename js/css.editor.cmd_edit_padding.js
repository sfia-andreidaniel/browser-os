function CSSEditor_cmd_edit_padding( app ) {
    
    app.handlers.cmd_edit_padding = function() {
    ...
    top.value = attributes['padding-top'];
    right.value = attributes['padding-right'];
    bottom.value = attributes['padding-bottom'];
    left.value = attributes['padding-left'];
    ...
    if ( attributes['padding'].indexOf(' ') == -1 )
        all.click();
    ...
    $export("0005-lbl", (new DOMLabel("Modify all paddings", { "for": all })).setAttr("style", "top: 143px; left: 40px; width: auto; position: absolute; text-overflow: ellipsis; right: 20px"));
    ...
    dlg.ok = function() {
        attributes['padding-left'] = left.value;
        attributes['padding-top'] = top.value;
        attributes['padding-bottom'] = bottom.value;
        attributes['padding-right'] = right.value;
        attributes = undefined;
        dlg.close();
    }
