function MMC2_Users_grid( app ) {
    ...
    app.grid.delrow = function( primaryKey ) {
        ...
        DialogBox("Are you sure you want to delete user:\n\n" + row.cells[1].value, {
            "childOf": getOwnerWindow( app.parentNode ),
            "type": "warning",
            "caption": "Confirm user deletion",
            "modal": true,
            "buttons": {
                "Yes": function() {
                
                    var req = [];
                    req.addPOST('do', 'delete-user' );
                    req.addPOST('id', row.primaryKey );
                    var rsp = $_JSON_POST( app.handler, req );

                    if ( rsp == 'ok' ) {
                        getOwnerWindow( app.parentNode ).onCustomEvent("deleteItem", "cmd_users:" + row.primaryKey );
