...
class XTemplate {
     ...
     * XML Language File
     */
    protected $_lang_file = null;
    protected $_lang_msgs = array();
    
    
    public function stripLines($buffer) {
        if (defined('NO_STRIP_ENTERS')) return $buffer;
        $lines = str_replace("\r", '', $buffer);
        $lines = explode("\n", $lines);
        $preMode = 0;
        for ($i=0; $i<count($lines); $i++) {
        $lower = strtolower($lines[$i]);
        $preMode = (strpos($lower, "<pre") !== false) ? $preMode + 1 : $preMode;
        $preMode = (strpos($lower, "</pre>") !== false) ? $preMode -1 : $preMode;
        $preMode = (strpos($lower, "<textarea") !== false) ? $preMode + 1 : $preMode;
        $preMode = (strpos($lower, "</textarea>") !== false) ? $preMode - 1 : $preMode;
        $preMode = (strpos($lower, "<script") !== false) ? $preMode + 1 : $preMode;
        $preMode = (strpos($lower, "</script>") !== false) ? $preMode - 1 : $preMode;
        if ($preMode === 0) { 
        ...
        private function preg_parse_strings($buffer) {
            if (!count($this->_lang_msgs)) return $buffer;
            $b = $buffer;
            while (preg_match('/<msg:([a-z_0-9\-\.\:]+)>/', $b, $matches)) {
                if (!isset($this->_lang_msgs[$matches[1]])) 
                    die("Error: Lang msg ".$matches[1]." is not defined in xml lang file!");
                $b = str_replace("<msg:".$matches[1].">", $this->_lang_msgs[$matches[1]], $b);
            }
            return $b;
        }
        
        private function get_msg($msg_name) {
            return isset($this->_lang_msgs["$msg_name"]) ? $this->_lang_msgs["$msg_name"] : '';
        }
    ...
    public function __construct($file, $LangFile = null, $LangSection = "*", $tpldir = '', $files = null, $mainblock = 'main', $autosetup = true) {

        $this->_lang_file = $LangFile;
        
        if ($this->_lang_file !== null) {
            if (!file_exists($this->_lang_file))
            throw new Exception("XTemplate::__construct:: Error loading xml language file $this->_lang_file");
            $dom = new DOMDocument();
            $dom->loadXML(file_get_contents($this->_lang_file));
            $msgs = $dom->getElementsByTagName('msg');
            for ($i=0; $i<$msgs->length; $i++) {
                        $msg = $msgs->item($i);
            $name = $msg->hasAttribute('name') ? $msg->getAttribute('name') : false;
            $section = $msg->hasAttribute('section') ? $msg->getAttribute('section') : '*';
            
            if (($LangSection == '*' || $section == $LangSection || $section == '*') && $name !== false)
                $this->_lang_msgs["$name"] = $msg->nodeValue;
            }
        }

//         echo "<pre>"; print_r($this->_lang_msgs); die("</pre>");
    ...
    public function parse ($bname) {
        ...
        $copy = $this->preg_parse_strings($copy);
    ...
    */
    ...
    public function text ($bname = '') {
        ...
        return (!defined("STRIP_ENTERS") ? $text : $this->stripLines($text));
    }
